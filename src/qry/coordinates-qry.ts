...
function coordinates2Rdf(iri: NamedNode, latitude: number, longitude: number): Observable<Quad> {
    return of(
        quadOf(iri, namedNodeOf('wgs84_pos:lat'), variableOf(latitude.toString())), 
        quadOf(iri, namedNodeOf('wgs84_pos:long'), variableOf(longitude.toString()))
    )
}
